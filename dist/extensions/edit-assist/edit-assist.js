...
function getState() {
    const g = globalThis;
    return g[STATE_KEY] ?? { failureCount: 0, autoRecoveryMode: false };
}
function setState(s) {
...
function normalizeEditWhitespace(event) {
    if (!event.input.edits)
        return;
...
function fuzzyLocate(content, needle, minScore = 0.5) {
    ...
    if (bestScore < minScore || bestIdx === -1)
        return null;
...
function buildDiagnostic(content, needle) {
    const match = fuzzyLocate(content, needle, 0.3);
    if (!match)
        return `  "${truncate(needle, 60)}" - not found anywhere in the file`;
    const lines = content.split("\n");
...
async function applyWithRecovery(path, content, edits) {
    let current = content;
    const applied = [];
    ...
    const parts = ["Applied " + applied.length + " edit(s) to " + path];
    if (recoveryCount > 0)
        parts.push("Auto-recovered " + recoveryCount + " fuzzy match(es)");
    if (failCount > 0)
        parts.push("" + failCount + " edit(s) could not be matched");
...
async function applyWithDiagnostic(path, content, edits) {
    let current = content;
    const applied = [];
    const failed = [];
    for (const edit of edits) {
    ...
    return {
        content: [{ type: "text", text: "Applied all " + edits.length + " edit(s) to " + path }],
        details: { edits: edits.length, path },
    };
}
export default function editAssist(pi) {
    pi.on("session_start", () => {
    ...
    pi.registerTool({
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            if (edits.length === 0) {
                return { content: [{ type: "text", text: "No edits provided." }], details: {} };
            }
            return withFileMutationQueue(absolutePath, async () => {
                ...
                try {
                    ...
                    return {
                        content: [{ type: "text", text: "Cannot read " + params.path + ": file does not exist or is not readable." }],
                ...
                if (state.autoRecoveryMode) {
                    ...
                    if (!result.isError) {
                        setState({ failureCount: 0, autoRecoveryMode: false });
                        try {
                            ctx.ui.notify("edit: auto-recovery succeeded, back to normal", "info");
                        }
                        catch { /* no-op */ }
                    }
                    return result;
                ...
                if (result.isError) {
                    ...
                    if (state.failureCount >= 2) {
                        ...
                        try {
                            ctx.ui.notify("edit: auto-recovery primed for next attempt", "warning");
                        }
                        catch { /* no-op */ }
                    }
                    setState(state);
...
//# sourceMappingURL=edit-assist.js.map